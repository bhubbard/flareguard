...
for (const slug of SLUGS) {
  optional[`@flareguard/${slug}`] = manifest.version;
}
...
console.log(
  `[flareguard] Added ${SLUGS.length} optionalDependencies at version ${manifest.version}:`,
);
for (const slug of SLUGS) console.log(`  @flareguard/${slug}`);
