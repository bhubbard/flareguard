...
const exe = os === "win32" ? "flareguard.exe" : "flareguard";
...
fs.writeFileSync(
  ...
  `${JSON.stringify(
    {
      name: `@flareguard/${slug}`,
      version: pkg.version,
      description: `Prebuilt flareguard engine for ${slug}.`,
      license: pkg.license,
...
fs.writeFileSync(
  ...
  `# @flareguard/${slug}\n\n` +
    `Prebuilt \`flareguard\` binary for ${slug}. Installed automatically as an\n` +
    `optional dependency of [\`flareguard\`](https://www.npmjs.com/package/flareguard).\n` +
